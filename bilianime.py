...
def Crawler(url):
    ...
    for i in range(20):
        ...
        # score[i]=re.findall('<h4 class="score">(.*?)</h4>',response2.text)[0] 现在score放在第一层里面了

        # media_url[i]='https://'+re.findall('<a href="//(.*?)" target="_blank" class="media-cover"><!----></a>',response2.text)[0]
        # response3=requests.get(url=media_url[i],headers=headers)
        media_tags[i]=re.findall('"styles":(.*?),"actors"',response2.text)[0]
        
        playtime[i]= re.findall('已观看(.*?)次',response2.text)[0]
        # print(type(playtime[i]))
        if "亿" in playtime[i]:
